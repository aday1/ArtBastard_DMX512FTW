    ...
    directories.forEach(dir => {
        if (!fs_1.default.existsSync(dir)) {
            try {
                ...
                (0, index_1.log)(`Created directory: ${dir}`);
            }
            catch (error) {
                (0, index_1.log)(`Failed to create directory ${dir}: ${error}`);
            }
        }
    });
}
...
try {
    ...
    server.listen(port, () => {
        ...
        try {
            ...
            (0, index_1.log)(`ERROR initializing application: ${error instanceof Error ? error.message : String(error)}`);
            if (error instanceof Error && error.stack) {
                (0, index_1.log)(`Stack trace: ${error.stack}`);
            }
        }
    });
    ...
    server.on('error', (error) => {
        if (error.code === 'EADDRINUSE') {
            (0, index_1.log)(`ERROR: Port ${port} is already in use. Please close any applications using this port and try again.`);
        }
        else {
            (0, index_1.log)(`SERVER ERROR: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
}
catch (error) {
    (0, index_1.log)(`FATAL ERROR initializing Socket.IO: ${error instanceof Error ? error.message : String(error)}`);
    if (error instanceof Error && error.stack) {
        (0, index_1.log)(`Stack trace: ${error.stack}`);
    }
}
