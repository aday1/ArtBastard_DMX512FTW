...
export const MidiLearnButton = ({ channelIndex, className }) => {
    ...
    return (_jsxs("button", { className: `${styles.learnButton} ${getButtonClass()} ${className || ''}`, onClick: handleClick, title: hasMapping ? 'Click to remap or right-click to remove' : 'Click to assign MIDI control', onContextMenu: (e) => {
            e.preventDefault();
            if (hasMapping) {
                // Remove mapping on right-click
                useStoreUtils.getState().removeMidiMapping(channelIndex);
            }
        }, children: [isChannelLearning && learnStatus === 'learning' && (_jsx("div", { className: styles.pulsingDot })), _jsx("span", { children: getButtonText() })] }));
};
